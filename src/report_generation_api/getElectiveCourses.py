...
def extract_cybersecurity_concentration_courses_from_html(html_doc: str) -> List[Dict[str, str]]:
    """
    Extract courses under:
    'Computer Systems Engineering (Cybersecurity) Concentration Requirements'

    Returns tagged items so callers can distinguish:
    - direct_required: direct course rows in the subsection
    - focus_course: rows inside 'Upper Division Cybersecurity Focus Courses'
    - elective: rows inside 'Cybersecurity Elective'

    Each item has shape: {"line": "<course line>", "tag": "<tag>"}
    """
    if not html_doc:
        return []
    ...
    while current_row:
        ...
        if "checksheet-requirement" in row_classes:
            ...
            # Direct (non-list) required course row
            if td and not has_list:
                direct_anchor = td.find("a", class_="ttCourse")
                if direct_anchor:
                    direct_text = _normalize_space(
                        direct_anchor.get_text(" ", strip=True)
                    ).strip(' "')
                    if direct_text and direct_text not in seen:
                        seen.add(direct_text)
                        items.append({"line": direct_text, "tag": "direct_required"})

            # Tagged lists inside the subsection
            for label in current_row.select("span.course-list-name"):
                list_name = _normalize_space(label.get_text(" ", strip=True)).lower()

                tag: Optional[str] = None
                if "focus courses" in list_name:
                    tag = "focus_course"
                elif "cybersecurity elective" in list_name:
                    tag = "elective"
                else:
                    continue
                ...
                for course_row in course_list_content.select("div.courseListRow"):
                    ...
                    if text and text not in seen:
                        seen.add(text)
                        items.append({"line": text, "tag": tag})
...
def extract_cybersecurity_concentration_courses_from_url(
    url: str, timeout: int = 30
) -> List[Dict[str, str]]:
    """Fetch URL HTML and extract tagged cybersecurity concentration course lines."""
    headers = {"User-Agent": "Mozilla/5.0", "Accept": "text/html"}
    response = requests.get(url, headers=headers, timeout=timeout)
    response.raise_for_status()
    return extract_cybersecurity_concentration_courses_from_html(response.text)
...
def build_merged_courses_json_from_html(
    html_doc: str, cyber_html_doc: str
) -> List[Dict[str, Any]]:
    """
    Build merged JSON from:
    - CSE technical elective list
    - Cybersecurity concentration requirements (direct + focus + elective)

    Rules:
    - dedupe exact same course (code, number, name)
    - tag each course with its source(s) in a `tags` list
    - if a course appears as cybersecurity direct_required or focus_course, set required_for = 'Cbs'
    """
    technical_courses = extract_cse_technical_electives_from_html(html_doc)
    cybersecurity_items = extract_cybersecurity_concentration_courses_from_html(cyber_html_doc)
    ...
    for line in technical_courses:
        ...
        if not parsed:
            continue
        parsed["tags"] = ["cse_technical_elective"]
        merged[_course_key(parsed)] = parsed

    for item in cybersecurity_items:
        line = item.get("line", "")
        tag = item.get("tag", "")
        ...
        if key in merged:
            tags = merged[key].get("tags") or []
            if isinstance(tags, list) and tag:
                tags.append(tag)
                merged[key]["tags"] = tags
        else:
            parsed["tags"] = [tag] if tag else []
            merged[key] = parsed

        if tag in ("direct_required", "focus_course"):
...
def build_merged_cse_courses_json_from_url(year: int, timeout: int = 30) -> List[Dict[str, Any]]:
