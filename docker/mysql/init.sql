...
CREATE TABLE IF NOT EXISTS faculty_workload (
    ...
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    CONSTRAINT chk_workload_pct_range CHECK (
        teaching_pct <= 100
        AND research_or_scholarship_pct <= 100
        AND other_pct <= 100
        AND pct_time_devoted_to_program <= 100
    ),
    CONSTRAINT chk_workload_sum CHECK (
        teaching_pct + research_or_scholarship_pct + other_pct = 100
    )
);
...
CREATE TABLE IF NOT EXISTS outcome_attainment_level (
    ...
    -- e.g. '70/70', empty cell = no row stored
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (program_id) REFERENCES programs(program_id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS continuous_improvement (
    ...
    type ENUM(
        ...
        'concentration_flowchart',
        'adhoc'
    ) NOT NULL,
    semester_year VARCHAR(50),
    -- only relevant for semester_improvement type
    source TEXT,
    problem_analysis TEXT,
    actions_plans TEXT,
    status_actions TEXT,
    result TEXT,
    -- only relevant for peo_update type
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (program_id) REFERENCES programs(program_id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS curriculum (
    ...
    course_type ENUM('R', 'E', 'SE') NOT NULL,
...
CREATE TABLE IF NOT EXISTS curriculum_peo_alignment (
    ...
    -- 1-4, could be auto increment too
...
CREATE TABLE IF NOT EXISTS curriculum_outcome_alignment (
    alignment_id INT AUTO_INCREMENT PRIMARY KEY,
    program_id INT NOT NULL,
    student_outcome TEXT NOT NULL,
    -- 1-7 with theor outcome description
    year_level VARCHAR(255) NOT NULL,
    courses TEXT,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (program_id) REFERENCES programs(program_id) ON DELETE CASCADE
...
-- Symfony-owned syllabus template approval workflow.
-- course_syllabi remains report data; these tables preserve immutable faculty
-- submissions, coordinator edits, and review decisions independently.
CREATE TABLE IF NOT EXISTS syllabus_common_courses (
    id INT AUTO_INCREMENT PRIMARY KEY,
    program_id INT NOT NULL,
    current_approved_revision_id INT NULL,
    course_subject VARCHAR(50) NOT NULL,
    course_number VARCHAR(50) NOT NULL,
    course_name VARCHAR(255) NOT NULL,
    ...
    UNIQUE KEY uniq_syllabus_common_course (program_id, course_subject, course_number, delivery_type),
...
CREATE TABLE IF NOT EXISTS syllabus_template_reviews (
    id INT AUTO_INCREMENT PRIMARY KEY,
    submission_id INT NOT NULL,
...
ALTER TABLE syllabus_common_courses
    ADD CONSTRAINT fk_syllabus_current_revision
        FOREIGN KEY (current_approved_revision_id) REFERENCES syllabus_template_revisions(id) ON DELETE SET NULL;
